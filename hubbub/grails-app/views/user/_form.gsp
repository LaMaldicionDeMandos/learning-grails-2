<%@ page import="org.pasut.learning.grails.User" %>
...
<div class="fieldcontain ${hasErrors(bean: userInstance, field: 'profile', 'error')} ">
	...
	<g:select id="profile" name="profile.id" from="${org.pasut.learning.grails.Profile.list()}" optionKey="id" value="${userInstance?.profile?.id}" class="many-to-one" noSelection="['null': '']"/>
...
<div class="fieldcontain ${hasErrors(bean: userInstance, field: 'following', 'error')} ">
	...
	<g:select name="following" from="${org.pasut.learning.grails.User.list()}" multiple="multiple" optionKey="id" size="5" value="${userInstance?.following*.id}" class="many-to-many"/>
